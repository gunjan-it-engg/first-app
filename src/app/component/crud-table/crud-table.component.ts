...
export class CrudTableComponent implements OnInit {
  addUpdatecom : boolean;
  addUpdatecomCall: any;
  ...
  constructor(public getEmp: EmployeService, public dialog: MatDialog) {
    // getting the value for update the table after add getting change signal from the service with the help of rxjs
    this.addUpdatecom = getEmp.addUpdate
    this.addUpdatecomCall = getEmp.addUpdatevisible.subscribe((value) =>{
      this.getEmployee()
      this.addUpdatecom = value
    })
  }
  ...
  getEmployee() {
    this.getEmp.getEmployee().subscribe({
      next: (res) => {
  ...
  editEmployee(row: any) {
    this.dialog
      .open(CrudDialogComponent, { data: row })
      .afterClosed()
      .subscribe((val) => {
        if (val == 'update') {
          this.getEmployee();
